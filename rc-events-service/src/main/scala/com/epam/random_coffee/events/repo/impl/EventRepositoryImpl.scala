...
class EventRepositoryImpl(transactor: Transactor[IO])(implicit runtime: IORuntime) extends EventRepository {
  ...
  override def save(event: Event): Future[Unit] = unsafeRun {
    sql"insert into event (id, name) values (${event.id.value}, ${event.eventName})".update.run.map(_ => ())
  }

  override def find(id: EventId): Future[Option[Event]] = unsafeRun {
    sql"select id, name from event where id = ${id.value}".query[Event].option
  }

  override def update(id: EventId, newEventName: String): Future[Unit] = unsafeRun {
    sql"update event set name = $newEventName where id = ${id.value}".update.run.map(_ => ())
  }
